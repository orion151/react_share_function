...
import { Button, InjectedModalProps, Skeleton, Text } from '@binance/uikit'
...
const ChangeProfilePicPage: React.FC<ChangeProfilePicPageProps> = ({ onDismiss }) => {
  ...
  const {
    ...
    onApprove: () => {
      return contract.methods.approve(getbinanceProfileAddress(), selectedNft.tokenId).send({ from: account })
    },
    onConfirm: () => {
      if (!profile.isActive) {
    ...
    <>
      ...
      {!isLoading && nftsInWallet.length === 0 && (
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {t('Make sure you have a binance Collectible in your wallet and try again!')}
          </Text>
        </>
      )}
